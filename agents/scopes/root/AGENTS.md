...
- Current scoped subagents:
  ...
  - `lib/services/AGENTS.md` -> core services specialist

## Reviewer Subagent Trigger
- If the prompt includes `review`, `code review`, or `reviewer`, switch to reviewer mode.
- Reviewer mode output must list findings first, ordered by severity, with file references and concrete risk.
- Prioritize functional bugs, regressions, and missing tests over style-only feedback.
- If no issues are found, explicitly state that and note any residual risks or untested paths.
